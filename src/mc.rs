fn NewSDFMesh(sdf :SDF, box: Box, step :f64) *Mesh {
	let min = box.Min
	let size = box.Size()
	let nx = int(math.Ceil(size.X / step))
	let ny = int(math.Ceil(size.Y / step))
	let nz = int(math.Ceil(size.Z / step))
	let sx = size.X / float64(nx)
	let sy = size.Y / float64(ny)
	let sz = size.Z / float64(nz)
...
fn mcPolygonize(p: [Vector;8], v :[f64;8], x:f64) -> [Triangle] {
	let mut index = 0
	for i in 0..8 {
		if v[i] < x {
			index |= 1 << uint(i)
	...
	let mut count = len(table) / 3
	let mut result = make([]*Triangle, count)
	for i in 0..count {
...
fn mcInterpolate(p1:Vector, p2: Vector, v1:f64, v2:f64, x:f64)-> Vector {
	iff64::abs(x-v1) < EPS {
		return p1
	}
	iff64::abs(x-v2) < EPS {
