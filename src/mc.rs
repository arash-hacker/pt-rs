...
// pub fn NewSDFMesh(sdf :Box<dyn SDF>, bx: BBox, step :f64) ->Mesh {
// 	let min = bx.Min;
// 	let size = bx.Size();
// 	let nx = (f64::ceil(size.X / step)) as i32;
// 	let ny = (f64::ceil(size.Y / step)) as i32;
// 	let nz = (f64::ceil(size.Z / step)) as i32;
// 	let sx = size.X / (nx) as f64;
// 	let sy = size.Y / (ny) as f64;
// 	let sz = size.Z / (nz) as f64;
// 	let  triangles:Vec<Triangle>=vec![];
// 	for x in 0..nx-1 {
// 		for y in 0..ny-1 {
// 			for z in 0..nz-1 {
// 				let (x0, y0, z0) = ((x as f64)*sx+min.X, (y as f64)*sy+min.Y, (z as f64)*sz+min.Z);
// 				let (x1, y1, z1) =( x0+sx, y0+sy, z0+sz);
// 				let p = [
// 					Vector{X:x0, Y:y0, Z:z0},
// 					Vector{X:x1, Y:y0, Z:z0},
// 					Vector{X:x1, Y:y1, Z:z0},
// 					Vector{X:x0, Y:y1, Z:z0},
// 					Vector{X:x0, Y:y0, Z:z1},
// 					Vector{X:x1, Y:y0, Z:z1},
// 					Vector{X:x1, Y:y1, Z:z1},
// 					Vector{X:x0, Y:y1, Z:z1},
// 				];
// 				let mut v :[f64;8];
// 				for i in 0..8 {
// 					v[i] = sdf.Evaluate(p[i]);
// 				}
// 				triangles.append(&mut mcPolygonize(p, v, 0.0));
// 			}
// 		}
// 	}
...
// pub fn mcPolygonize(p: [Vector;8], v :[f64;8], x:f64) -> Vec<Triangle> {
// 	let mut index = 0;
// 	for i in 0..8 {
// 		if v[i] < x {
// 			index |= 1 << (i) as u8
...
// 	let mut table = triangleTable[index];
// 	let mut count = (table).len() / 3;
// 	let mut result: Vec<Triangle>;
// 	for i in 0..count {
...
// pub fn mcInterpolate(p1:Vector, p2: Vector, v1:f64, v2:f64, x:f64)-> Vector {
// 	if f64::abs(x-v1) < common::EPS {
// 		return p1
// 	}
// 	if f64::abs(x-v2) < common::EPS {
// 		return p2
// 	}
// 	if f64::abs(v1-v2) < common::EPS {
// 		return p1
// 	}
// 	let t = (x - v1) / (v2 - v1);
// 	return Vector{
...
// pub static triangleTable:Vec<Vec<i32>> = vec![
// 	vec![],
// 	vec![0, 8, 3],
// 	vec![0, 1, 9],
// 	vec![1, 8, 3, 9, 8, 1],
